...
extern "C" int  mv_nd (int n, int nc, double *soln, int bound, bool min, bool instreward);
extern "C" void  datacpy_nd (int n, int *row_starts, int *choice_starts, double *non_zeros, int nc, double *soln, unsigned int *cols, double * yes_vec);
extern "C" void datafree_nd ();
...
)
{
	...
	if (use_gpu) {
		// Use row starts and choice starts
		if (use_counts) {
	...
	if (use_gpu) {
		...
		datacpy_nd (n,row_starts,choice_starts,non_zeros,nc,soln,cols,yes_vec);

		iters = mv_nd (n,nc,soln,bound,min,false);
