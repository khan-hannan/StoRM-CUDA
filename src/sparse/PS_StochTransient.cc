...
extern "C" int  mv_stbu (int n, bool compact_tr, bool compact_d,
						...
						int term_crit, double term_crit_param,
...
)
{
	...
	if (use_gpu) {
		...
		// datacpy to GPU
		datacpy_stbu (n, compact_tr, compact_d, col_starts, non_zeros, soln, rows, dist, dist_mask,
	...
	if (use_gpu) {
		...
		iters = mv_stbu (n,compact_tr,compact_d,sum,dist_mask,dist_shift,fgw.left, fgw.right, fgw.weights,
						term_crit, term_crit_param_unif, unif, do_ss_detect, FALSE);
