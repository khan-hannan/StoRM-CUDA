...
extern "C" int  mv_ndu (int n, int nc, double *soln,
						int bound, int term_crit, double term_crit_param, bool min, int export_adv, int *adv);
extern "C" void  datacpy_ndu (int n, int *row_starts, int *choice_starts, double *non_zeros, int nc, double *soln, unsigned int *cols,
							double *yes_vec, int export_adv, int *adv);
extern "C" void datafree_ndu ();
...
//		if (soln[i]) printf("yes[%d] := %f;\n", i+1, yes[i]);
	...
	if (use_gpu) {
		...
		datacpy_ndu (n,row_starts,choice_starts,non_zeros,nc,soln,cols,
					yes_vec, ((export_adv_enabled != EXPORT_ADV_NONE) ? 1 : 0),
					adv);

		iters = mv_ndu (n,nc,soln,max_iters,term_crit,term_crit_param,min,((export_adv_enabled != EXPORT_ADV_NONE) ? 1 : 0),adv);
...
//		PS_PrintToMainLog(env, "%.2f %.2f sec\n", ((double)(util_cpu_time() - start3)/1000), ((double)(util_cpu_time() - start2)/1000)/iters);
	...
	//if (use_counts && use_gpu) {delete[] row_starts; delete[] choice_starts; }
