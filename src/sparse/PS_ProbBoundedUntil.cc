...
extern "C" int  mv_pbu (int n, bool compact_tr, bool compact_y, double *soln,
						int dist_mask, int dist_shift, int bound, bool use_yes);
extern "C" void  datacpy_pbu (int n, bool compact_tr, bool compact_y, double * yes_vec, DistVector * yes_dist,
			int *row_starts, double *non_zeros, double *soln, unsigned int *cols,double * dist,
			int dist_mask, int dist_shift);
extern "C" void datafree_pbu (bool compact_tr, bool compact_y);
...
)
{
	...
	if (use_gpu) {
		...
		datacpy_pbu(n,compact_tr,compact_y, yes_vec, yes_dist, row_starts, non_zeros, soln, cols,
									dist,dist_mask,dist_shift);
		clock_t end_datacpy = clock();

		double time_datacpy = (double)(end_datacpy - start_datacpy) / CLOCKS_PER_SEC;
    	fprintf(stdout, "datacpy_pbu execution time: %f seconds\n", time_datacpy);

		// matrix multiply
		clock_t start_mv = clock();
		iters = mv_pbu(n, compact_tr, compact_y, soln, dist_mask, dist_shift, bound, TRUE);
		clock_t end_mv = clock();
    	double time_mv = (double)(end_mv - start_mv) / CLOCKS_PER_SEC;
    	fprintf(stdout, "mv_pbu execution time: %f seconds\n", time_mv);

		datafree_pbu(compact_tr, compact_y);
