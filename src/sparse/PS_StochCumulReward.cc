...
extern "C" int  mv_stbu (int n, bool compact_tr, bool compact_d,
						double *sum, int dist_mask, int dist_shift, int left, int bound,
						double * weights, int term_crit, double term_crit_param,
...
)
{
	...
	if (use_gpu) {
		...
		double unif = setup_stbu (n, distnum, compact_tr, compact_d, diags, diags_dist);
	}
	...
	if (use_gpu) {
		...
		iters = mv_stbu (n,compact_tr,compact_d,sum,dist_mask,dist_shift,fgw.left, fgw.right, fgw.weights,
						term_crit, term_crit_param_unif, unif, do_ss_detect, TRUE);
