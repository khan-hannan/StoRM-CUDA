...
extern "C" int  mv_pcr (int n, bool compact_tr, bool compact_r, double *soln,
						int dist_mask, int dist_shift, int bound);
extern "C" void  datacpy_pcr (int n, bool compact_tr, bool compact_r, double * rew_vec, DistVector * rew_dist,
			int *row_starts, double *non_zeros, double *soln, unsigned int *cols,double * dist,
			int dist_mask, int dist_shift);
extern "C" void datafree_pcr (bool compact_tr, bool compact_r);
...
)
{
	...
	if (use_gpu) {
		...
		datacpy_pcr(n,compact_tr,compact_r, rew_vec, rew_dist, row_starts, non_zeros, soln, cols,
									dist,dist_mask,dist_shift);
		// matrix multiply
		iters = mv_pcr(n, compact_tr, compact_r, soln, dist_mask, dist_shift, bound);
		
		datafree_pcr(compact_tr, compact_r);
