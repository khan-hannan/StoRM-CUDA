...
extern "C" int  mv_pt (int n, bool compact_tr, double *soln, int dist_mask, int dist_shift,
			int bound, int term_crit, double term_crit_param, bool do_ss_detect);
extern "C" void  datacpy_pir (int n, bool compact_tr,
			int *row_starts, double *non_zeros, double *soln, unsigned int *cols,double * dist,
			int dist_mask, int dist_shift);
extern "C" void datafree_pir (bool compact_tr);
...
)
{
	...
	if (use_gpu) {
		...
		datacpy_pir(n,compact_tr, col_starts, non_zeros, soln, rows,
									dist,dist_mask,dist_shift);
		// matrix multiply
		iters = mv_pt(n, compact_tr, soln, dist_mask,dist_shift, time, term_crit, term_crit_param, do_ss_detect);
		
		datafree_pir(compact_tr);
