...
extern "C" int  mv_stbu (int n, bool compact_tr, bool compact_d,
						double *sum, int dist_mask, int dist_shift, int left, int bound, double * weights,
						int term_crit, double term_crit_param, double unif, bool do_ss_detect, bool cumulreward);
extern "C" void  datacpy_stbu (int n, bool compact_tr, bool compact_d,
...
extern "C" double setup_stbu (int n, long nnz, bool compact_tr, bool compact_d, double * diags_vec, DistVector * diags_dist);
extern "C" void datafree_stbu (bool compact_tr, bool compact_d);
...
)
{
	...
	if (use_gpu) {
		...
		clock_t start_datacpy = clock();
		// datacpy to GPU
		datacpy_stbu (n, compact_tr, compact_d, row_starts, non_zeros, soln, cols, dist, dist_mask,
					dist_shift, diags, diags_dist);
		...
		if (!compact_tr) {
			...
			distnum = cmsrsm->dist_num;
		}
		// Perform setup on GPU
		unif = setup_stbu (n, distnum, compact_tr, compact_d, diags, diags_dist);

		clock_t end_datacpy = clock();

		double time_datacpy = (double)(end_datacpy - start_datacpy) / CLOCKS_PER_SEC;
    	fprintf(stdout, "datacpy execution time: %f seconds\n", time_datacpy);
	}
	...
	if (use_gpu) {
		PS_PrintToMainLog(env, "\nStarting GPU iterations...\n");
		fgw.right = time;
		clock_t start_mv = clock();
		iters = mv_stbu (n,compact_tr,compact_d,sum,dist_mask,dist_shift,
									fgw.left, fgw.right, fgw.weights, term_crit, term_crit_param_unif, unif, do_ss_detect, FALSE);

		clock_t end_mv = clock();
    	double time_mv = (double)(end_mv - start_mv) / CLOCKS_PER_SEC;
    	fprintf(stdout, "mv_stbu execution time: %f seconds\n", time_mv);

		datafree_stbu (compact_tr, compact_d);
		fprintf (stdout, "GPU memory freed\n");
	}
	else {
		PS_PrintToMainLog(env, "\nStarting iterations...\n");
...
//		PS_PrintToMainLog(env, "%.2f %.2f sec\n", ((double)(util_cpu_time() - start3)/1000), ((double)(util_cpu_time() - start2)/1000)/iters);
	...
	//if (use_counts && use_gpu) delete[] row_starts;
