...
extern "C" int  mv (int n, void * b,bool compact_a, bool compact_b , bool compact_d, double *soln,
							int dist_mask, int dist_shift, jdouble omega,
							int max_iters, int term_crit, double term_crit_param, bool power);
extern "C" void  datacpy_power (int n, void * b, bool compact_a, bool compact_b, double * b_vec, DistVector * b_dist, int *row_starts,
							double *non_zeros, double *soln, unsigned int *cols, double *dist, int dist_mask, int dist_shift);
extern "C" void  datafree_power (bool compact_a, bool compact_b);
...
)
{
	...
	if (use_gpu) {
		...
		datacpy_power(n,(void * )b,compact_a,compact_b,b_vec,b_dist,row_starts,non_zeros,soln,cols,dist,dist_mask,dist_shift);
		// matrix multiply
		iters = mv(n,(void * )b,compact_a,compact_b,NULL,soln,dist_mask,dist_shift,NULL,max_iters,term_crit,term_crit_param,true);
		done = (iters > 0 && iters < max_iters);
		
		datafree_power(compact_a, compact_b);
