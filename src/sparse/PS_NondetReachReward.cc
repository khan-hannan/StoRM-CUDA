...
extern "C" int  mv_ndr (int n, int nc, double *soln, int nc_r,
						int bound, int term_crit, double term_crit_param, bool min, int export_adv, int *adv, double huge);
extern "C" void  datacpy_ndr (int n, int *row_starts, int *choice_starts, double *non_zeros, int nc, double *soln, unsigned int *cols,
							int *choice_starts_r, double *non_zeros_r, int nc_r, unsigned int *cols_r,
							double *sr_vec, double *inf_vec, int export_adv, int *adv);
extern "C" void datafree_ndr ();
...
)
{
	...
	if (use_gpu) {
		// Use row starts, choice starts, and choice starts rewards
		if (use_counts) {
			row_starts = new int[n+1];
			row_starts[0] = 0;
			choice_starts = new int[nc];
			for (int i=1; i<n+1; i++) {
				row_starts[i] = row_starts[i-1] + row_counts[i-1];
			}
			choice_starts[0] = row_starts[0];
			for (int i=1; i<nc; i++) {
				choice_starts[i] = choice_starts[i-1] + choice_counts[i-1];
			}
		}
		if (use_counts_r) {
			choice_starts_r = new int[nc_r];
			choice_starts_r[0] = row_starts[0];
			for (int i=1; i<nc_r; i++) {
	...
	if (use_gpu) {
		...
		datacpy_ndr (n,row_starts,choice_starts,non_zeros,nc,soln,cols,
					choice_starts_r, non_zeros_r, nc_r, cols_r, sr_vec, inf_vec, ((export_adv_enabled != EXPORT_ADV_NONE) ? 1 : 0),
					adv);

		iters = mv_ndr (n,nc,soln,nc_r,max_iters,term_crit,term_crit_param,min,((export_adv_enabled != EXPORT_ADV_NONE) ? 1 : 0),adv,HUGE_VAL);
...
//		PS_PrintToMainLog(env, "%.2f %.2f sec\n", ((double)(util_cpu_time() - start3)/1000), ((double)(util_cpu_time() - start2)/1000)/iters);
	...
	//if (use_counts && use_gpu) { delete[] row_starts; delete[] choice_starts; delete[] choice_starts_r; }
