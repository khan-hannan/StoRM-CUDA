...
template<>
void SparseMatrixBuilder<double>::addCudaChunk() {
    // Calculate the size of the chunk to copy
    double copySize = columnsAndValues.size() - GlobalCuda->LastSize;

    // Check if the chunk size is within the fixed length limit
    if (copySize <= GlobalCuda->FixedLen) {
        return;
    }

    // Allocate memory if the copy size exceeds the last copied size
    if (copySize > GlobalCuda->lastcopysize) {
        GlobalCuda->HostMemCopyAlloc(copySize);
    }

    // Adjust fixed length and iteration length
    if (GlobalCuda->IterationLen > 4) {
        GlobalCuda->FixedLen *= 5;
        GlobalCuda->IterationLen = 0;
    } else {
        GlobalCuda->IterationLen++;
    }

    GlobalCuda->lastcopysize = copySize;
    index_type iter = 0;

    // Copy column and value data to GPU buffers
    for (int i = GlobalCuda->LastSize; i < columnsAndValues.size(); ++i) {
        GlobalCuda->Cols[iter] = columnsAndValues[i].getColumn();
        GlobalCuda->Vals[iter] = columnsAndValues[i].getValue();
        iter++;
    }

    // Add the chunk to the CUDA memory
    GlobalCuda->AddChunk(copySize);
    GlobalCuda->LastSize = columnsAndValues.size();
}

template<>
void SparseMatrixBuilder<double>::flushCudaChunks() {
    // Calculate the size of the chunk to copy
    double copySize = columnsAndValues.size() - GlobalCuda->LastSize;
    ...
    // Allocate host memory for flushing
    GlobalCuda->HostAllocFlush(rowIndications.size());
    index_type iter = 0;
    ...
    // Copy row indications to GPU buffers
    for (int i = 0; i < rowIndications.size(); ++i) {
        GlobalCuda->hA_csrOffsets[i] = rowIndications[i];
    }
    GlobalCuda->hA_csrOffsets[rowIndications.size()] = columnsAndValues.size();

    // Add rows to the CUDA memory
    GlobalCuda->AddRow(rowIndications.size());
    GlobalCuda->LastSize = columnsAndValues.size();
}

// Template specialization for non-double types
template<typename ValueType>
void SparseMatrixBuilder<ValueType>::flushCudaChunks() {
    std::cout << "only for double datatype" << std::endl;
}

template<typename ValueType>
void SparseMatrixBuilder<ValueType>::addCudaChunk() {
    std::cout << "only for double datatype" << std::endl;
}
